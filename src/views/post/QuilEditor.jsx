      ...
      urlMatches.forEach((link) => {
        Axios.get(link)
          .then((payload) => {
            ...
            const rendered = `<a href=${conf.BACKEND_URL + '/' + url} target="_blank"><div><img crossorigin="anonymous" src=${conf.BACKEND_URL + '/' + image} alt=${title} width="20%"/><span>${title}</span></div></a>`;
...
class ImageBlot extends BlockEmbed {
  static create(value) {
    ...
    imgTag.setAttribute('src', conf.BACKEND_URL + '/' + value.src);
...
class VideoBlot extends BlockEmbed {
  static create(value) {
    if (value && value.src) {
      ...
      videoTag.setAttribute('src',conf.BACKEND_URL +"/"+  value.src);
      ...
      iframeTag.setAttribute('src',conf.BACKEND_URL +"/"+  value);
...
class QuillEditor extends React.Component {
        ...
        .then((response) => {
          if (response.data?.success) {
            ...
            quill.insertEmbed(position, 'image', {
              src: conf.BACKEND_URL + '/' + response.data.url,
  ...
  insertVideo = (e) => {
    ...
    if (e.currentTarget && e.currentTarget.files && e.currentTarget.files.length > 0) {
      ...
      formData.append('file', file);
      Axios.post('/files', formData, config).then((response) => {
        if (response.data?.success) {
          const quill = this.reactQuillRef.getEditor();
          quill.focus();
  ...
  formats = ['header', 'bold', 'italic', 'underline', 'strike', 'image', 'link', 'code-block', 'blockquote', 'clean'];
}
